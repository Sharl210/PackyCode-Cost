...
2. 复制到全局插件目录（或放到项目级目录 `.opencode/plugins/`）：
...
OpenCode 官方推荐在 Windows 使用 WSL。请在 WSL 中按上面的 Linux 步骤安装与配置。

文档：https://opencode.ai/docs/windows-wsl

### 方式 B：在 opencode.json 显式加载

`opencode.json` 支持 `plugin` 配置（来源：https://opencode.ai/docs/plugins）。

全局配置路径：`~/.config/opencode/opencode.json`（来源：https://opencode.ai/docs/config）。
